BeforeAll {
    ...
    # Pester must also resolve the external commands before mocking them in YardstickSupport's scope.
...
Describe "New-SupersedenceObject" {
    BeforeEach {
        Mock -ModuleName YardstickSupport New-YardstickWin32AppSupersedence {
            [ordered]@{
...
Describe "Set-YardstickSupersedence" {
    BeforeEach {
        ...
        Mock -ModuleName YardstickSupport Remove-YardstickWin32AppSupersedence {}
        Mock -ModuleName YardstickSupport New-YardstickWin32AppSupersedence {
            [ordered]@{ 'targetId' = $ID; 'supersedenceType' = $SupersedenceType.ToLower() }
        }
        Mock -ModuleName YardstickSupport Add-YardstickWin32AppSupersedence {
    ...
    It "attaches one supersedence entry per target" {
        ...
        $count | Should -Be 2
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppSupersedence -Times 1 -Exactly
    }
    ...
    It "returns 0 and skips Add when there are no targets" {
        ...
        $count | Should -Be 0
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppSupersedence -Times 0 -Exactly
    }
    ...
    It "clears stale links on the new app when there are no targets" {
        ...
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppSupersedence -Times 1 -Exactly -ParameterFilter { $ID -eq 'new' }
    }
    ...
    It "never trims away an update-only anchor when capping" {
        ...
        $count | Should -Be 9
        Should -Invoke -ModuleName YardstickSupport New-YardstickWin32AppSupersedence -Times 1 -Exactly -ParameterFilter { $ID -eq 'anchor' -and $SupersedenceType -eq 'Update' }
    }
    ...
    It "forces Update for ids listed in UpdateOnlyIds even when Type is Replace" {
        ...
        Should -Invoke -ModuleName YardstickSupport New-YardstickWin32AppSupersedence -Times 1 -Exactly -ParameterFilter { $ID -eq 'anchor' -and $SupersedenceType -eq 'Update' }
        Should -Invoke -ModuleName YardstickSupport New-YardstickWin32AppSupersedence -Times 1 -Exactly -ParameterFilter { $ID -eq 'old' -and $SupersedenceType -eq 'Replace' }
    }
    ...
    It "strips stale forward links off targets before re-parenting them" {
        ...
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppSupersedence -Times 1 -Exactly -ParameterFilter { $ID -eq 'old' }
    }
}
...
Describe "Remove-YardstickApp" {
    BeforeEach {
        Mock -ModuleName YardstickSupport Remove-YardstickWin32App {}
        Mock -ModuleName YardstickSupport Remove-YardstickWin32AppSupersedence {}
        Mock -ModuleName YardstickSupport Remove-SupersedenceReference {}
        Mock -ModuleName YardstickSupport Remove-YardstickWin32AppDependency {}
        ...
        Mock -ModuleName YardstickSupport Invoke-YardstickGraphRequest { @() }
        Mock -ModuleName YardstickSupport Get-YardstickWin32AppDependency { @() }
        # Deletion succeeded: the app is no longer resolvable.
        Mock -ModuleName YardstickSupport Get-YardstickWin32App { $null }
    }
    ...
    It "detaches superseding parents before deleting" {
        ...
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32App -Times 1 -Exactly -ParameterFilter { $Id -eq 'doomed' }
    }

    It "clears its own forward links before deleting" {
        $Global:MockSupersedenceCleared = $false
        Mock -ModuleName YardstickSupport Remove-YardstickWin32AppSupersedence { $Global:MockSupersedenceCleared = $true }
        Mock -ModuleName YardstickSupport Get-YardstickSupersedenceRelationship {
            if ($Global:MockSupersedenceCleared) { return @() }
            @([PSCustomObject]@{ sourceId = 'doomed'; targetId = 'older' })
        }
        Remove-YardstickApp -App ([PSCustomObject]@{ id = 'doomed'; DisplayName = 'App (N-3)' })
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppSupersedence -Times 1 -Exactly -ParameterFilter { $ID -eq 'doomed' }
    ...
    It "makes no relationship calls when the app is unrelated" {
        Mock -ModuleName YardstickSupport Get-YardstickSupersedenceRelationship { @() }
        Remove-YardstickApp -App ([PSCustomObject]@{ id = 'doomed'; DisplayName = 'App (N-3)' })
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppSupersedence -Times 0 -Exactly
        Should -Invoke -ModuleName YardstickSupport Remove-SupersedenceReference -Times 0 -Exactly
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32App -Times 1 -Exactly
    }

    It "throws when Intune silently refuses the delete" {
        Mock -ModuleName YardstickSupport Get-YardstickSupersedenceRelationship { @() }
        Mock -ModuleName YardstickSupport Get-YardstickWin32App {
    ...
    It "does not attempt the delete when a link survives the unwind" {
        ...
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32App -Times 0 -Exactly
    }
...
Describe "Clear-YardstickAppLink" {
    ...
    It "deletes every assignment by its own id" {
        # By-id is the point: Remove-YardstickWin32AppAssignmentGroup matches on the
    ...
    It "clears the app's own dependencies" {
        ...
        Mock -ModuleName YardstickSupport Remove-YardstickWin32AppDependency { $Global:MockDependenciesCleared = $true }
        Mock -ModuleName YardstickSupport Get-YardstickWin32AppDependency {
        ...
        $surviving = @(Clear-YardstickAppLink -App ([PSCustomObject]@{ id = 'doomed'; DisplayName = 'App (N-3)' }) -RetryDelaySeconds 0)
        $surviving | Should -BeNullOrEmpty
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppDependency -Times 1 -Exactly -ParameterFilter { $ID -eq 'doomed' }
    }
    ...
    It "treats a read it could not complete as a surviving link" {
        Mock -ModuleName YardstickSupport Get-YardstickWin32AppDependency { throw "Authentication token was not found" }
        $surviving = @(Clear-YardstickAppLink -App ([PSCustomObject]@{ id = 'doomed'; DisplayName = 'App (N-3)' }) -RetryDelaySeconds 0)
...
Describe "Remove-DependencyReference" {
    BeforeEach {
        Mock -ModuleName YardstickSupport Remove-YardstickWin32AppDependency {}
        Mock -ModuleName YardstickSupport Add-YardstickWin32AppDependency {}
        Mock -ModuleName YardstickSupport New-YardstickWin32AppDependency {
    ...
    It "clears dependencies entirely when the removed target was the only one" {
        ...
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppDependency -Times 1 -Exactly -ParameterFilter { $ID -eq 'parent' }
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppDependency -Times 0 -Exactly
    }
    ...
    It "rebuilds the remaining targets when other dependencies survive" {
        ...
        Remove-DependencyReference -ParentId 'parent' -TargetId 'doomed'
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppDependency -Times 1 -Exactly -ParameterFilter {
            $Dependency.Count -eq 1 -and $Dependency[0].targetId -eq 'keeper' -and $Dependency[0].dependencyType -eq 'AutoInstall'
        }
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppDependency -Times 0 -Exactly
    }
...
Describe "Remove-SupersedenceReference" {
    ...
    It "clears supersedence entirely when the removed target was the only one" {
        ...
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppSupersedence -Times 1 -Exactly -ParameterFilter { $ID -eq 'parent' }
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppSupersedence -Times 0 -Exactly
    }
    ...
    It "rebuilds the remaining targets when other links survive" {
        ...
        Remove-SupersedenceReference -ParentId 'parent' -TargetId 'doomed'
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppSupersedence -Times 1 -Exactly -ParameterFilter {
...
Describe "Move-AssignmentsAndDependencies intent split" {
    BeforeEach {
        Mock -ModuleName YardstickSupport Get-YardstickWin32AppAssignment {
            @(
        ...
        Mock -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup {
            [PSCustomObject]@{ id = 'new-assignment' }
        }
        Mock -ModuleName YardstickSupport Remove-YardstickWin32AppAssignmentGroup {}
    }
    ...
    It "IntentFilter='required' moves only required and removes from source" {
        ...
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup -Times 1 -Exactly -ParameterFilter { $Intent -eq 'required' }
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup -Times 0 -Exactly -ParameterFilter { $Intent -eq 'available' }
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppAssignmentGroup -Times 1 -Exactly -ParameterFilter { $GroupID -eq 'g-req' }
    }
    ...
    It "IntentFilter='required' with -SkipDependencies moves required and leaves dependencies alone" {
        ...
        # set must not be merged onto the current app, where Add-YardstickWin32AppDependency
        ...
        Mock -ModuleName YardstickSupport New-YardstickWin32AppDependency { [ordered]@{ targetId = $ID; dependencyType = $DependencyType } }
        Mock -ModuleName YardstickSupport Add-YardstickWin32AppDependency {}
        Mock -ModuleName YardstickSupport Remove-YardstickWin32AppDependency {}
        ...
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup -Times 1 -Exactly -ParameterFilter { $Intent -eq 'required' }
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppAssignmentGroup -Times 1 -Exactly -ParameterFilter { $ID -eq 'anchor' -and $GroupID -eq 'g-req' }
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppDependency -Times 0 -Exactly
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppDependency -Times 0 -Exactly
    }
    ...
    It "migrates by id, so a source whose rename has not propagated still works" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';     DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to -IntentFilter 'required' -SkipDependencies
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup -Times 1 -Exactly -ParameterFilter { $ID -eq 'to' }
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppAssignmentGroup -Times 1 -Exactly -ParameterFilter { $ID -eq 'anchor' }
    }
    ...
    It "IntentFilter='available' with CopyOnly copies available and leaves source untouched" {
        ...
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup -Times 1 -Exactly -ParameterFilter { $Intent -eq 'available' }
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup -Times 0 -Exactly -ParameterFilter { $Intent -eq 'required' }
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppAssignmentGroup -Times 0 -Exactly
    }
    ...
    It "no filter processes every intent" {
        ...
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup -Times 2 -Exactly
    }

    It "keeps the source assignment when the target did not actually receive it" {
        # Add-YardstickWin32AppAssignmentGroup downgrades Graph failures to warnings
        ...
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup -Times 3 -Exactly
    }
    ...
    It "-CopyOnly:`$false falls back to a move" {
        ...
        Should -Invoke -ModuleName YardstickSupport Add-YardstickWin32AppAssignmentGroup -Times 1 -Exactly -ParameterFilter { $Intent -eq 'available' }
        Should -Invoke -ModuleName YardstickSupport Remove-YardstickWin32AppAssignmentGroup -Times 1 -Exactly -ParameterFilter { $GroupID -eq 'g-avail' }
    }
}

Describe "Move-AssignmentsAndDependencies child dependencies" {
    BeforeEach {
        # Add-YardstickWin32AppDependency replaces an app's whole dependency set, so
        ...
        Mock Get-YardstickWin32AppDependency -ModuleName YardstickSupport {
            switch ($ID) {
                'from' {
                    @(
        ...
        Mock New-YardstickWin32AppDependency -ModuleName YardstickSupport {
            [ordered]@{ targetId = $ID; dependencyType = $DependencyType }
        }
        Mock Add-YardstickWin32AppDependency -ModuleName YardstickSupport {
        ...
        Mock Remove-YardstickWin32AppDependency -ModuleName YardstickSupport {}
    }
    ...
    It "submits every child dependency in a single Add so none are clobbered" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to
        Should -Invoke Add-YardstickWin32AppDependency -ModuleName YardstickSupport -Exactly -Times 1
    ...
    It "never passes a relationship id where an app id is expected" {
        ...
        Should -Invoke Get-YardstickWin32AppDependency -ModuleName YardstickSupport -Exactly -Times 0 -ParameterFilter { $ID -like '*_*' }
        Should -Invoke New-YardstickWin32AppDependency -ModuleName YardstickSupport -Exactly -Times 0 -ParameterFilter { $ID -like '*_*' }
        Should -Invoke Add-YardstickWin32AppDependency -ModuleName YardstickSupport -Exactly -Times 0 -ParameterFilter { $ID -like '*_*' }
    }
    ...
    It "only ever writes dependencies to the target app" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to
        Should -Invoke Add-YardstickWin32AppDependency -ModuleName YardstickSupport -Exactly -Times 0 -ParameterFilter { $ID -ne 'to' }
        Should -Invoke Remove-YardstickWin32AppDependency -ModuleName YardstickSupport -Exactly -Times 0
    }
    ...
    It "normalizes the dependency type to the values New-YardstickWin32AppDependency accepts" {
        $from = [PSCustomObject]@{ id = 'from'; DisplayName = 'App (N-1)' }
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to
    ...
    It "drops a dependency the target already holds on an app being deleted this run" {
        # Add-YardstickWin32AppDependency replaces the whole set, so re-submitting an
    ...
    It "does not touch dependencies when SkipDependencies is set" {
        ...
        Should -Invoke Add-YardstickWin32AppDependency -ModuleName YardstickSupport -Exactly -Times 0
        Should -Invoke Remove-YardstickWin32AppDependency -ModuleName YardstickSupport -Exactly -Times 0
    }

    It "retries when the target app does not report the dependencies back" {
        # Write never lands - Add-YardstickWin32AppDependency warns instead of
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to
        Should -Invoke Add-YardstickWin32AppDependency -ModuleName YardstickSupport -Exactly -Times 3
    }
}

Describe "Get-YardstickAppAssignment" {
    It "drops the all-null assignment the cmdlet invents for an app with none" {
        # Get-YardstickWin32AppAssignment projects the empty OData envelope into one
...
Describe "Move-AssignmentsAndDependencies assignment targets" {
    BeforeEach {
        ...
        Mock Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport { [PSCustomObject]@{ id = 'new-assignment' } }
        Mock Add-YardstickWin32AppAssignmentAllDevices -ModuleName YardstickSupport { [PSCustomObject]@{ id = 'new-assignment' } }
        Mock Add-YardstickWin32AppAssignmentAllUsers -ModuleName YardstickSupport { [PSCustomObject]@{ id = 'new-assignment' } }
        Mock Remove-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport {}
        Mock Remove-YardstickWin32AppAssignmentAllDevices -ModuleName YardstickSupport {}
        Mock Remove-YardstickWin32AppAssignmentAllUsers -ModuleName YardstickSupport {}
    }
    ...
    It "migrates an All Devices assignment instead of skipping it" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to -SkipDependencies
        Should -Invoke Add-YardstickWin32AppAssignmentAllDevices -ModuleName YardstickSupport -Exactly -Times 1 -ParameterFilter { $ID -eq 'to' -and $Intent -eq 'required' -and $Notification -eq 'hideAll' }
        Should -Invoke Remove-YardstickWin32AppAssignmentAllDevices -ModuleName YardstickSupport -Exactly -Times 1 -ParameterFilter { $ID -eq 'from' }
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 0
    }
    ...
    It "migrates an All Users assignment instead of skipping it" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to -SkipDependencies
        Should -Invoke Add-YardstickWin32AppAssignmentAllUsers -ModuleName YardstickSupport -Exactly -Times 1 -ParameterFilter { $ID -eq 'to' -and $Intent -eq 'available' }
        Should -Invoke Remove-YardstickWin32AppAssignmentAllUsers -ModuleName YardstickSupport -Exactly -Times 1 -ParameterFilter { $ID -eq 'from' }
    }
    ...
    It "leaves a filtered All Devices assignment alone rather than widening it to every device" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to -SkipDependencies
        Should -Invoke Add-YardstickWin32AppAssignmentAllDevices -ModuleName YardstickSupport -Exactly -Times 0
        Should -Invoke Remove-YardstickWin32AppAssignmentAllDevices -ModuleName YardstickSupport -Exactly -Times 0
    }
    ...
    It "keeps a group exclusion an exclusion" {
        ...
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 1 -ParameterFilter { $Exclude -eq $true -and $GroupID -eq 'g-excl' -and $Intent -eq 'required' }
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 0 -ParameterFilter { $Include -eq $true }
    }
    ...
    It "keeps a group inclusion an inclusion" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to -SkipDependencies
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 1 -ParameterFilter { $Include -eq $true -and $GroupID -eq 'g-incl' }
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 0 -ParameterFilter { $Exclude -eq $true }
        Should -Invoke Remove-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 1 -ParameterFilter { $ID -eq 'from' -and $GroupID -eq 'g-incl' }
    }
    ...
    It "carries a group assignment's filter across" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to -SkipDependencies
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 1 -ParameterFilter { $FilterID -eq 'filter-1' -and $FilterMode -eq 'exclude' }
    }
    ...
    It "still skips targets it has no way to migrate" {
        ...
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 0
        Should -Invoke Add-YardstickWin32AppAssignmentAllDevices -ModuleName YardstickSupport -Exactly -Times 0
        Should -Invoke Add-YardstickWin32AppAssignmentAllUsers -ModuleName YardstickSupport -Exactly -Times 0
    }
    ...
    It "leaves the source untouched under CopyOnly" {
        ...
        Should -Invoke Add-YardstickWin32AppAssignmentAllDevices -ModuleName YardstickSupport -Exactly -Times 1
        Should -Invoke Remove-YardstickWin32AppAssignmentAllDevices -ModuleName YardstickSupport -Exactly -Times 0
    }
}
...
Describe "Test-YardstickAssignmentPresent" {
    ...
    It "finds an assignment on an app that holds exactly one" {
        # Get-YardstickWin32AppAssignment returns $null in this case under Windows
...
Describe "Move-AssignmentsAndDependencies assignment error handling" {
    ...
    It "records the Intune warning in the Yardstick log instead of only the console" {
        Mock Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport {
    ...
    It "treats an 'already exists' conflict as success when the assignment really is there" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to -SkipDependencies -RetryDelaySeconds 0
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Times 1 -Exactly
        Should -Invoke Remove-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Times 1 -Exactly
    }
    ...
    It "never strips the source under -CopyOnly, even when Intune says the target already has it" {
        ...
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Times 1 -Exactly
        Should -Invoke Remove-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Times 0 -Exactly
    }
    ...
    It "retries a transient failure and keeps the source when it never lands" {
        ...
        $to   = [PSCustomObject]@{ id = 'to';   DisplayName = 'App' }
        Move-AssignmentsAndDependencies -From $from -To $to -SkipDependencies -RetryDelaySeconds 0
        Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Times 3 -Exactly
        Should -Invoke Remove-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Times 0 -Exactly
    }
    ...
    It "recovers when a retry succeeds after a transient failure" {
        ...
        try {
            Move-AssignmentsAndDependencies -From $from -To $to -SkipDependencies -RetryDelaySeconds 0
            Should -Invoke Add-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Times 2 -Exactly
            Should -Invoke Remove-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Times 1 -Exactly
        }
        finally {
...
Describe "Move-AssignmentsAndDependencies break containment" {
    It "keeps migrating later assignments when a cmdlet bails out with break" {
        ...
        try {
            ...
            # function to reproduce what the YardstickWin32App cmdlets actually do
            ...
            function global:Add-YardstickWin32AppAssignmentGroup {
                [CmdletBinding()]
            ...
            Remove-Item function:global:Add-YardstickWin32AppAssignmentGroup -ErrorAction SilentlyContinue
        }
        ...
        Should -Invoke Remove-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 0 -ParameterFilter { $GroupID -eq 'g-bails' }
        Should -Invoke Remove-YardstickWin32AppAssignmentGroup -ModuleName YardstickSupport -Exactly -Times 1 -ParameterFilter { $GroupID -eq 'g-after' }
