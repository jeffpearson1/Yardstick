    ...
    if ($FromAssignments) {
        foreach ($Assignment in $FromAssignments) {
            ...
            if ($IntentFilter -and ($Assignment.Intent -ne $IntentFilter)) {
                Write-Log "Skipping assignment for group $($Assignment.GroupID) - intent $($Assignment.Intent) does not match filter $IntentFilter"
                continue
            }
            $maxRetries = 3
            $try = 0
            ...
            # Exclusion-group assignments (and assignments created outside Yardstick)
            # come back with no notification setting, which
            # Add-IntuneWin32AppAssignmentGroup rejects outright.
            $Notification = if ($Assignment.Notifications) { $Assignment.Notifications } else { 'showAll' }
                ...
                if ($Assignment.FilterType -eq "none") {
                    if ($Assignment.InstallTimeSettings) {
                        if ($startDateTime -and $deadlineDateTime) {
                            ...
                            try {
                                Add-IntuneWin32AppAssignmentGroup -Include -ID $To.id -GroupID $Assignment.GroupID -Intent $Assignment.Intent -Notification $Notification -AvailableTime $startDateTime -DeadlineTime $deadlineDateTime -UseLocalTime $useLocalTime | Out-Null
                        ...
                        elseif ($startDateTime) {
                            ...
                            try {
                                Add-IntuneWin32AppAssignmentGroup -Include -ID $To.id -GroupID $Assignment.GroupID -Intent $Assignment.Intent -Notification $Notification -AvailableTime $startDateTime -UseLocalTime $useLocalTime | Out-Null
                                $successfullyAdded = $true
                            }
                            catch {
                                Write-Log "Failed to add assignment to $($To.id) for group $($Assignment.GroupID): $_"
                        ...
                        elseif ($deadlineDateTime) {
                            ...
                            try {
                                Add-IntuneWin32AppAssignmentGroup -Include -ID $To.id -GroupID $Assignment.GroupID -Intent $Assignment.Intent -Notification $Notification -DeadlineTime $deadlineDateTime -UseLocalTime $useLocalTime | Out-Null
                                $successfullyAdded = $true
                            }
                            catch {
                                Write-Log "Failed to add assignment to $($To.id) for group $($Assignment.GroupID): $_"
                        ...
                        try {
                            Add-IntuneWin32AppAssignmentGroup -Include -ID $To.id -GroupID $Assignment.GroupID -Intent $Assignment.Intent -Notification $Notification | Out-Null
                            $successfullyAdded = $true
                        }
                        catch {
                            Write-Log "Failed to add assignment to $($To.id) for group $($Assignment.GroupID): $_"
                    ...
                    if ($startDateTime -and $deadlineDateTime) {
                        Write-Log "Adding assignment to $($To.id) for group $($Assignment.GroupID) with deadline and available time settings."
                        try {
                            Add-IntuneWin32AppAssignmentGroup -Include -ID $To.id -GroupID $Assignment.GroupID -Intent $Assignment.Intent -Notification $Notification -AvailableTime $startDateTime -DeadlineTime $deadlineDateTime -UseLocalTime $useLocalTime -FilterMode $Assignment.FilterType -FilterID $Assignment.FilterID | Out-Null
                            $successfullyAdded = $true
                        } catch {
                                Write-Log "Failed to add assignment to $($To.id) for group $($Assignment.GroupID): $_"
                        }
                        ...
                        Write-Log "Adding assignment to $($To.id) for group $($Assignment.GroupID) with available time settings."
                        try {
                            Add-IntuneWin32AppAssignmentGroup -Include -ID $To.id -GroupID $Assignment.GroupID -Intent $Assignment.Intent -Notification $Notification -AvailableTime $startDateTime -UseLocalTime $useLocalTime -FilterMode $Assignment.FilterType -FilterID $Assignment.FilterID | Out-Null
                            $successfullyAdded = $true
                        } catch {
                                Write-Log "Failed to add assignment to $($To.id) for group $($Assignment.GroupID): $_"
                        }
                        ...
                        Write-Log "Adding assignment to $($To.id) for group $($Assignment.GroupID) with deadline time settings."
                        try {
                            Add-IntuneWin32AppAssignmentGroup -Include -ID $To.id -GroupID $Assignment.GroupID -Intent $Assignment.Intent -Notification $Notification -DeadlineTime $deadlineDateTime -UseLocalTime $useLocalTime -FilterMode $Assignment.FilterType -FilterID $Assignment.FilterID | Out-Null
                            $successfullyAdded = $true
                        } catch {
                                Write-Log "Failed to add assignment to $($To.id) for group $($Assignment.GroupID): $_"
                        }
                        ...
                        Write-Log "Adding assignment to $($To.id) for group $($Assignment.GroupID) without time settings."
                        try {
                                Add-IntuneWin32AppAssignmentGroup -Include -ID $To.id -GroupID $Assignment.GroupID -Intent $Assignment.Intent -Notification $Notification -FilterMode $Assignment.FilterType -FilterID $Assignment.FilterID | Out-Null
                                $successfullyAdded = $true
                        } catch {
                                Write-Log "Failed to add assignment to $($To.id) for group $($Assignment.GroupID): $_"
                        }
            ...
            if ($successfullyAdded -and -not $CopyOnly) {
                # Add-IntuneWin32AppAssignmentGroup swallows Graph failures and
                # duplicate-target conflicts as warnings, so confirm the target
                # really carries the assignment before deleting the source.
                $verified = $false
                try {
                    $verified = @(Get-IntuneWin32AppAssignment -Id $To.id | Where-Object {
                        ($_.GroupID -eq $Assignment.GroupID) -and ($_.Intent -eq $Assignment.Intent)
                    }).Count -gt 0
                } catch {
                    Write-Log "Could not verify assignment on $($To.id) for group $($Assignment.GroupID): $_"
                }
                if (-not $verified) {
                    Write-Log "ERROR: Assignment for group $($Assignment.GroupID) was not found on $($To.id) after the add; leaving the source assignment on $($From.id) intact."
                    continue
                }
                for ($i = 0; $i -le 3; $i++) {
                    try {
